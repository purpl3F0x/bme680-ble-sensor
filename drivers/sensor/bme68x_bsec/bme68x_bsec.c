 ...
 * @brief Zephyr Integration for BME68X + BSEC
 * @copyright Copyright (c) 2024 Stavros Avramidis |2023 Nordic Semiconductor ASA
 */
...
LOG_MODULE_REGISTER(bsec, CONFIG_BME68X_BSEC_LOG_LEVEL);
...
#include "generic_33v_3s_4d/bsec_iaq.txt"
};

/* Temperature offset due to external heat sources. */
static const float temp_offset = (CONFIG_BME68X_BSEC_TEMPERATURE_OFFSET / (float)100);
...
static const bsec_sensor_configuration_t bsec_requested_virtual_sensors[] = {
    ...
    */
    ...
    /*
    Stabilisation BSEC_OUTPUT_STABILIZATION_STATUS
    */
    {
...
static K_THREAD_STACK_DEFINE(thread_stack, CONFIG_BME68X_BSEC_THREAD_STACK_SIZE);

/* Used for a timeout for when BSEC's state should be saved. */
static K_TIMER_DEFINE(bsec_save_state_timer, NULL, NULL);

/* sensor DT Spec, either i2c_dt_spec or spi_dt_spec*/
static union sensor_bus bme68x_dt_spec = {
#if BME680_BUS_SPI
    .spi = SPI_DT_SPEC_INST_GET(0, BME680_SPI_OPERATION, 0),
#elif BME680_BUS_I2C
    .i2c = I2C_DT_SPEC_INST_GET(0),
#endif
};

static struct bme68x_bsec_data data_0;
...
{
    ARG_UNUSED(key);
    struct bme68x_bsec_data* data = param;
...
{
    int ret;
    struct bme68x_bsec_data* data = &data_0;

    LOG_DBG("saving state to flash");

    ret = bsec_get_state(0, data->state_buffer, ARRAY_SIZE(data->state_buffer), data->work_buffer,
        ARRAY_SIZE(data->work_buffer), &data->state_len);
...
static int8_t bus_write(uint8_t reg_addr, const uint8_t* reg_data_ptr, uint32_t len, void* intf_ptr)
{
    ...
    return spi_write_dt(&bme68x_dt_spec.spi, &tx);
    ...
    return i2c_write_dt(&bme68x_dt_spec.i2c, buf, ARRAY_SIZE(buf));
...
static int8_t bus_read(uint8_t reg_addr, uint8_t* reg_data_ptr, uint32_t len, void* intf_ptr)
{
    ...
    return spi_transceive_dt(&bme68x_dt_spec.spi, &tx, &rx);

#elif BME680_BUS_I2C
    return i2c_write_read_dt(&bme68x_dt_spec.i2c, &reg_addr, 1, reg_data_ptr, len);
#endif
}
...
static void output_ready(const struct device* dev, const bsec_output_t* outputs, uint8_t n_outputs)
{
    ...
    for (size_t i = 0; i < n_outputs; ++i) {
        ...
        case BSEC_OUTPUT_IAQ:
            data->latest.iaq = (uint16_t)outputs[i].signal;
            LOG_DBG("IAQ: %d", data->latest.iaq);
            break;

        case BSEC_OUTPUT_CO2_EQUIVALENT:
            data->latest.co2Equivalent = (float)outputs[i].signal;
            LOG_DBG("CO2 Eq: %.2f", data->latest.co2Equivalent);
            break;

        case BSEC_OUTPUT_BREATH_VOC_EQUIVALENT:
            data->latest.breathVocEquivalent = (float)outputs[i].signal;
            LOG_DBG("VOC Eq: %.2f", data->latest.breathVocEquivalent);
            break;
        ...
        case BSEC_OUTPUT_STABILIZATION_STATUS:
            data->latest.stabStatus = outputs[i].signal != 0.0f;
            LOG_DBG("Stabilisation Status: %u", data->latest.stabStatus);
            break;

        case BSEC_OUTPUT_RUN_IN_STATUS:
            data->latest.runInStatus = outputs[i].signal != 0.0f;
            LOG_DBG("Run-In Status: %u ", data->latest.runInStatus);
            break;
    ...
    data->latest.timestamp = outputs[0].time_stamp;
...
static int apply_sensor_settings(const struct device* dev, bsec_bme_settings_t sensor_settings)
{
    ...
    case BME68X_PARALLEL_MODE:
        ...
        heater_config.shared_heatr_dur = BSEC_TOTAL_HEAT_DUR - (bme68x_get_meas_dur(sensor_settings.op_mode, &config, &data->dev) / INT64_C(1000));
    ...
    uint64_t timestamp_ns)
{
    ...
    for (size_t i = 0; i < n_fields; ++i) {
        ...
        n_inputs = sensor_data_to_bsec_inputs(*sensor_settings, sensor_data + i, inputs, timestamp_ns);
    ...
    while (true) {
        ...
        if (k_timer_remaining_get(&bsec_save_state_timer) == 0) {
            ...
            k_timer_start(&bsec_save_state_timer,
                K_MINUTES(CONFIG_BME68X_BSEC_SAVE_INTERVAL_MINUTES),
                K_NO_WAIT);
        }
...
static int bme68x_bsec_init(const struct device* dev)
{
    int err;
    struct bme68x_bsec_data* data = &data_0;
    // const struct bme68x_bsec_config *config = dev->config;
...
#if BME680_BUS_SPI
    if (!spi_is_ready_dt(&bme68x_dt_spec.spi)) {
        LOG_ERR("SPI device not ready");
        return -ENODEV;
    }
    LOG_DBG("Found bme68x on SPI");
    data->dev.intf = BME68X_SPI_INTF;

#elif BME680_BUS_I2C
    if (!device_is_ready(bme68x_dt_spec.i2c)) {
        LOG_ERR("I2C device not ready");
        return -ENODEV;
    }
    LOG_DBG("Found bme68x on I2C");
    ...
    data->dev.amb_temp = CONFIG_BME68X_BSEC_EXPECTED_AMBIENT_TEMP;
    ...
    err = bsec_set_configuration(bsec_config_iaq, ARRAY_SIZE(bsec_config_iaq), data->work_buffer,
        ARRAY_SIZE(data->work_buffer));
    if (err != BSEC_OK) {
        LOG_ERR("Failed to set BSEC configuration: %d", err);
    }
    ...
    k_thread_create(&data->thread, thread_stack, CONFIG_BME68X_BSEC_THREAD_STACK_SIZE,
        (k_thread_entry_t)bsec_thread_fn, (void*)dev, NULL, NULL,
        K_LOWEST_APPLICATION_THREAD_PRIO, 0, K_NO_WAIT);

    k_timer_start(&bsec_save_state_timer, K_MINUTES(CONFIG_BME68X_BSEC_SAVE_INTERVAL_MINUTES),
        K_NO_WAIT);
    return 0;
}
...
SENSOR_DEVICE_DT_INST_DEFINE(0, bme68x_bsec_init, NULL, &data_0, NULL, POST_KERNEL,
